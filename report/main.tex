...
LCD 显示模块在 DA Wave 页面绘制 DAC 双通道波形预览曲线，在 Dashboard 仪表盘页面同时显示电参量数值和 ADC 采样波形。DA Wave 页面将 128 点 DAC 样点（0--4095）线性映射到图形区域 y 坐标，采用逐点连线方式绘制，坐标轴含纵轴（幅值方向）、横轴（零线）和刻度标记，CH1 用绿色、CH2 用黄色区分显示。Dashboard 仪表盘页面上半区绘制电压和电流通道的 ADC 采样波形曲线（电压通道黄色、电流通道青色），下半区以文本形式显示电压、电流、功率、漏电流、功率因数和校准状态，使电参数与波形实时一目了然。
...
LCD 采用 ALIENTEK TFT 屏（16 位并行接口，分辨率 240$\times$320），显示刷新由事件驱动触发，Dashboard 仪表盘页面以约 3 Hz 由 ADC 新帧事件刷新，其余页面在状态变化时即时刷新。波形图区域约 216$\times$120 像素，128 点波形经线性插值连线后视觉平滑。电参量以文本格式显示，显示分辨率分别为电压 0.01 V、电流 0.001 A、功率 0.1 W。
...
\centering
\begin{tikzpicture}[
    box/.style={blk, minimum width=2.2cm, minimum height=4.8cm, fill=black!3, draw=black, thick},
    subbox/.style={blk, minimum width=2.2cm, minimum height=4.8cm, fill=black!3, draw=black, thick},
    pinlabel/.style={rectangle, draw=black!80, fill=white, inner sep=2pt, font=\scriptsize\sffamily, minimum width=1.0cm}
]

% 左侧：STM32开发板 (标题置于框上方以防遮挡，并调整为黑白样式)
...
% 连接线及标注 (黑白设计，使用实线)
\draw[fl, thick, draw=black] (stm5v.east) -- (io5v.west) node[midway, above, lbl, text=black] {5V 供电};
\draw[fl2, thick, draw=black] (stmgnd.east) -- (iognd.west) node[midway, above, lbl, text=black] {共地};
\draw[fl, draw=black] (iofr.west) -- (stmpa1.east) node[midway, above, lbl, text=black] {市电频率 fr};
\draw[fl, draw=black] (iovl.west) -- (stmpc0.east) node[midway, above, lbl, text=black] {交流电压 VL};
\draw[fl, draw=black] (ioilk.west) -- (stmpc2.east) node[midway, above, lbl, text=black] {漏电电流 iLK};
\draw[fl, draw=black] (ioil.west) -- (stmpc3.east) node[midway, above, lbl, text=black] {负载电流 iL};
...
app\_ui：菜单状态机（6 个页面状态：仪表盘、测试菜单、PWM、频率测量、DA 波形、系统信息）
...
（7）调用全局数据读取函数 \texttt{app\_monitor\_state\_read()} 获取状态数据，若屏幕刷新标志 \texttt{display\_changed} 被置位，则调度屏幕显示任务 \texttt{app\_display\_task()} 刷新 LCD。LCD 刷新采用纯事件驱动机制，仅在有实际状态变化或 ADC 新帧（仪表盘页面）时触发，不再使用固定周期定时刷新。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/debug_online.jpg}
\caption{联机调试全景}
...
\textit{图 15\quad 联机调试实物及接线全景照片（备用预留）}\\[0.3cm]
（备用全景照片占位）
\vspace{3cm}}}
\caption{联机调试全景（备用）}
\label{fig:debug-online-backup}
\end{figure}
...
在团队协作方面，小组成员针对引脚分配、协议帧格式以及滤波算法的选择展开了充分讨论，通过将系统配置常量集中定义于 \texttt{app\_config.h}、协议帧结构规范化定义于 \texttt{app\_protocol\_defs.h}，有效保障了下位机固件与上位机分析工具之间的通信一致性。在调试及分析过程中，AI辅助工具在架构优化，分析数据竞争问题、排查栈溢出等复杂问题上提供了有益的指导，但针对菜单界面设计、交互逻辑、需求分析和具体的硬件寄存器操作、引脚分配问题，仍需人工进行规划、审查与测试验证。例如 PC1 电流通道读数异常的问题，需要人工查阅开发板资料并正确规划引脚分配。
