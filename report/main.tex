...
系统的交流测量信号均由 miniIO 副板进行调理。副板上集成了交流电压检测、交流电流检测、漏电电流检测、市电频率整形以及继电器、蜂鸣器控制等硬件模块，本设计使用其电压、电流、漏电流、整形测频及继电器控制接口。
...
由于 STM32 的模数转换通道输入电压范围为 0--3.3 V，因此 miniIO 副板必须使用 1.65 V 的参考电压将双极性交流信号抬升至单极性的安全输入范围。软件在计算电参量时会首先扣除该直流偏置，再计算各交流信号的有效值、功率及功率因数。另外，市电频率通过副板的电路转化为 0--3.3 V 方波信号 fr，输出至 PA1(TIM2\_CH2)，利用输入捕获测量电网频率。

模数转换采用 TIM3 触发 + ADC1 扫描 + DMA 循环传输的采样方案，详细设计见第~\ref{sec:adc}~章。DAC 产生的正弦波仅作为测试信号源，用于在不接入实际高压负载时验证采样和算法链路。

\itemline{5}{曲线绘制}

LCD 显示模块在 DA Wave 页面绘制 DAC 双通道波形预览曲线，在 Dashboard 仪表盘页面同时显示电参量数值和 ADC 采样波形。DA Wave 页面将 128 点 DAC 样点(0--4095)线性映射到图形区域 y 坐标，采用逐点连线方式绘制，坐标轴含纵轴(幅值方向)、横轴(零线)和刻度标记，CH1 用绿色、CH2 用黄色区分显示。Dashboard 仪表盘页面上半区绘制电压和电流通道的 ADC 采样波形曲线(电压通道黄色、电流通道青色)，下半区以文本形式显示电压、电流、功率、漏电流、功率因数及 PA1 实时测频值。

\itemline{6}{信号分析}

电参量计算采用整型算法实现。计算内容包括：电压有效值(VRMS)、电流有效值(IRMS)、漏电电流有效值、有功功率(P)、视在功率(S)和功率因数(PF)。所有物理量使用整数表示，无浮点运算。
...
频率测量分辨率由 TIM2 的 1 MHz 计数时钟决定，周期测量精度为 $\pm$1 $\mu$s。以 50 Hz 信号为例，理论周期 20000 $\mu$s。ADC 采样率 6400 Hz，每周期 128 点。满足 Nyquist 定理对 50 Hz，RMS 计算基于全周期采样。
...
(4)ADC 采样与数据拆分。ADC 采用三通道同步扫描、DMA 循环搬运与中断解交错的数据采集方案，详见第~\ref{sec:adc}~章。

(5)电参量整型计算。包括电压有效值(VRMS)、电流有效值(IRMS)、有功功率(P)、视在功率(S)和功率因数(PF)。算法采用去直流均方根和逐点功率积等数学方法，结合定点数缩放和 64 位累加防溢出设计，详细公式与定点数格式见第~\ref{sec:calculation}~章。
...
app\_screenshot：LCD 屏幕截图，读取 GDDRAM 显存数据并 RLE16 压缩，通过 TYPE=0x84 帧发送
...
频率测量模块通过第~\ref{sec:debug-protocol}~节所述的二进制帧协议与上位机通信。与频率相关的串口命令如表~\ref{tab:freq-commands}~所示，通过 PWM SET 和 STATUS? 命令验证本模块的频率设置与测量功能。
...
\begin{tikzpicture}[node distance=12mm, >=Latex]
  ...
  \node[blk, above=of t3] (t6) {TIM6 TRGO\\$f_s{=}f_{sig}{\times}128$};
  \node[io, right=of t3] (t5) {DAC 输出\\PA4 / PA5};
  \draw[fl] (t1)--(t2);
  \draw[fl] (t2)--(t3);
  \draw[fl] (t3)-- node[lbl,below]{DMA 搬运} (t5);
  \draw[fl] (t6.south) -- node[lbl,right]{触发} (t3.north);
\end{tikzpicture}}
...
ADC 采样与 DAC 输出模块通过第~\ref{sec:debug-protocol}~节所述的二进制帧协议与上位机通信。与 ADC/DAC 相关的串口命令如表~\ref{tab:adc-commands}~所示，通过 DAC SET 和 STATUS? 命令验证本模块的参数设置与采样功能。
...
其中 $G_v$、$G_i$ 为校准增益$\times$1000。电压通道增益 $G_v = 17600$(对应实际电压互感器与分压比)，电流通道增益 $G_i$ 由实测标定(默认 1000，即 1:1)。电压增益按实测比值整定，可通过修改 \texttt{APP\_ADC\_CALIB\_V\_GAIN\_X1000} 宏调整，无需重新编写算法。
...
电参量通过串口协议的 STATUS? 命令和自动上报功能传输至上位机，通信协议详见第~\ref{sec:debug-protocol}~节。
...
(1)PC1(ADC1\_IN11)电流通道读数异常。现象：DAC DUAL 环回测试时 IRMS 始终约为 0.056，远小于预期。排查发现 MiniSTM32 开发板的 PC1 与板载 LCD 共享引脚，硬件上拉电阻将 PC1 拉至约 3V，导致 ADC 读数严重偏高。解决办法：将电流采样通道迁移至 PC3(ADC1\_IN13)，避开触摸屏干扰。
...
(3)继电器未正确初始化。现象：miniIO 接入市电后，仪表盘各参数显示异常，大部分参数为零。解决办法：在 app\_init() 中增加继电器初始化逻辑，确保继电器上电后处于开启状态。
