...
本课程设计基于 STM32F103RCT6 微控制器，设计并实现了一套桌面用电监控系统。系统采用无操作系统的裸机前后台轮询结构，软件采用模块化设计：利用 PWM 输出产生测试频率，通过定时器输入捕获功能测量频率，使用双通道 DAC 生成测试正弦波，并对三通道 ADC 的交流信号进行同步采样与电参量计算；本地显示采用 TFT LCD 多级菜单，远程通信则通过 USART 二进制帧协议实现。

系统以 MiniSTM32 开发板为核心硬件平台。在信号生成与感知部分，利用 PA8/TIM1 输出占空比为 50\% 且频率可调的方波，并通过 PA1/TIM2 的输入捕获功能测量外部信号频率；利用 PA4/PA5 DAC 通道配合 DMA 循环搬运，输出 128 点的正弦波信号。在交流信号采集方面，PC0/PC3/PC2 三通道 ADC 由 TIM3 触发，以 6400 Hz 的采样率对电压、电流和漏电电流进行顺序扫描采样。三路通道共用同一触发源，其间转换时差极小，对于 50 Hz 基波信号的电参量计算影响可以忽略。采样数据由 DMA 传输至工作缓冲区后，程序调用整型算法计算电压与电流有效值、有功功率、视在功率及功率因数。通信协议采用分层设计，传输层以 A5 5A 帧头与 CRC-16/Modbus 校验保障传输可靠性，命令层则在帧内承载 ASCII 文本指令（支持 HELP、STATUS?、REPORT ON/OFF、PWM SET、DAC SET 等命令）。人机交互界面通过多页面状态机实现，包含主菜单与四个功能页面，且 LCD 显示与串口查询共享同一套全局状态数据，以确保两端显示数据的一致性。

系统调试表明，频率测量分辨率达到 0.01 Hz，PWM 设定范围为 1--100000 Hz，DAC 输出频率在 1--1000 Hz 内可调。在系统联调测试下，ADC 采样、电参量计算、显示刷新及串口通信等模块均运行稳定，实现了课程设计的各项功能要求。
...
测试频率信号由单片机的 PA8（TIM1\_CH1）产生，输出占空比为 50\% 的 PWM 方波。默认 50Hz，可经 LCD 菜单按键或串口命令（PWM SET）在 1--100000 Hz 范围内灵活调整。定时器配置为向上计数模式，通过动态计算预分频器（PSC）和自动重装载值（ARR）来覆盖宽频范围，比较捕获寄存器（CCR）则固定为 ARR 的一半以保证 50\% 占空比。

\itemline{2}{频率测量}

频率测量功能采用 PA1（TIM2\_CH2）的输入补获通道实现。TIM2 的计数器配置为 1 MHz 工作频率，即分辨率为 1 $\mu$s。测量程序记录相邻上升沿的捕获计数值差，换算为信号周期，再利用 $f = 100{,}000{,}000 / T_{\mu s}$ 得到以 0.01 Hz 为单位的整型频率值。此外，程序利用定时器溢出中断将计数值的高 16 位进行软件扩展，从而得到 32 位宽的时间戳，以支持对超低频信号的测量。

\itemline{3}{正弦波测试信号生成}

测试用正弦波信号由 PA4/PA5 通道的双路 DAC 产生。系统内部预先定义了 128 点全幅正弦查找表。在运行期间，程序根据设定的幅度和相位参数对查找表数据进行缩放与移位，并写入 DMA 缓冲区。由 TIM6 触发源驱动 DMA 循环将数据搬运至 DAC 寄存器输出。该模块支持单通道正弦（另一通道输出 1.65V 中点电压）及可调相位差的双通道正弦两种模式，输出频率在 1--1000 Hz 范围内可调，幅度码值在 0--2047 范围内可调。

\itemline{4}{桌面用电信号采集与信号处理电路}

系统的交流测量信号均由 miniIO 副板进行调理。副板上集成了交流电压检测、交流电流检测、漏电电流检测、市电频率整形以及继电器、蜂鸣器控制等硬件模块，本设计主要使用其电压、电流、漏电流及整形测频接口。

在电压检测链路上，市电电压信号经副板的电压互感器隔离并缩小后，进入运算放大器进行滤波调理，并叠加 1.65 V 直流偏置，最终生成的 VL 信号（以 1.65 V 为中心的交流信号）送入单片机的 PC0（ADC1\_IN10）通道。电流检测链路通过电流互感器将负载电流转换为小电流信号，经采样电阻转换为电压后，由运放调理并叠加 1.65 V 偏置输出 iL，送入 PC3（ADC1\_IN13）。漏电电流检测则利用零序互感器检测火线与零线的不平衡电流，经运放放大和偏置抬升后输出 iLK，送入 PC2（ADC1\_IN12）。

由于 STM32 的模数转换通道输入电压范围为 0--3.3 V，因此 miniIO 副板必须使用 1.65 V 的参考电压将双极性交流信号抬升至单极性安全输入范围。软件在计算电参量时会首先扣除该直流偏置，再计算各交流信号的有效值、功率及功率因数。另外，市电频率通过副板的整形电路转化为 0--3.3 V 方波信号 fr，输出至 PA1（TIM2\_CH2），利用输入捕获测量电网频率。

模数转换部分由 TIM3 触发 ADC1 进行规则组扫描采样，采样频率为 6400 Hz（每周期 128 点）。转换结果由 DMA1 循环传输至内存中（缓冲区大小为两倍帧长），并在 DMA 半传输（HT）与传输完成（TC）中断中由软件对交错的数据进行通道解交错拆分，以实现无缝的连续信号采集。DAC 产生的正弦波仅作为测试信号源，用于在不接入实际高压负载时验证采样和算法链路。
...
USART1 配置为 115200 bps、8N1 格式，通过 CH340 USB 转串口芯片连接上位机。协议帧超时保护 300 ms，半帧间隔超时自动重置解析器。环形缓冲区溢出检测保证高负载下不会进入错误状态。自动上报功能可按固定周期（约 1 秒）向串口推送 STATUS 全局状态数据，便于实时监控。
...
系统主控采用 STM32F103RCT6 微控制器，不依赖实时操作系统（RTOS），而是采用前后台的裸机轮询架构。系统上电后，引导程序和初始化函数会依次配置板级外设安全电平、初始化 LCD 屏、按键输入接口、串口通信协议栈、配置测频与 PWM 发生相关定时器、以及 DAC、ADC 和 UI 菜单状态机，准备就绪后进入系统调度主循环。
...
（4）运行采样处理任务 \texttt{app\_adc\_task()}，若检测到 DMA 搬运并分离出的 ADC 新数据帧，则将其载入主循环的工作缓冲区，然后调用整型算法库计算最新的电压、电流等电参数。
...
（7）调用全局数据读取函数 \texttt{app\_monitor\_state\_read()} 获取状态数据，若屏幕刷新标志 \texttt{display\_changed} 被置位，则调度屏幕显示任务 \texttt{app\_display\_task()} 刷新 LCD。
...
系统设计了双重交流信号来源以适应调试与实际应用。第一种是内置的 DAC 生成的模拟正弦信号，用于在不连接 miniIO 副板和高压负载时，闭环验证单片机自身的采样、DMA 传输、电参数计算算法以及人机交互界面的完整数据链路。第二种是 miniIO 副板的实际采样输出，包含调理后的电压信号 VL、电流信号 iL 和漏电流信号 iLK 以及整形频率信号 fr。在系统实际接入桌面负载时，监控系统直接切换至 miniIO 输入源，而 DAC 正弦波则主要起闭环自测作用。
...
\textbf{幅值缩放控制：}用户可动态调整正弦波幅度，程序对表中的每个正弦样点进行定点数缩放：

$$S'[n] = 2048 + (S[n] - 2048) \times A / 2047$$

其中 $A$ 为用户设定的幅值参数（范围 0--2047），运算结果在输出前进行限幅，以防数据溢出。

\textbf{通道相位差控制：}在双通道正弦输出模式下，通道 2 相对于通道 1 的相位偏移通过查表时的索引移位实现。设相位角差为 $\phi$（0--359度），则对应的样本偏移索引为 $\Delta = \phi \times 128 / 360$，通道 2 输出第 $n$ 个样点时，其实际读取的查找表索引为 $(n + \Delta) \bmod 128$。

\textbf{DMA 硬件传输：}当配置为单通道正弦（SINGLE 模式）时，DMA2 通道 3 将 16 位正弦波样本搬运至 DAC 单通道右对齐寄存器 DHR12R1，DMA2 通道 4 则向通道 2 寄存器 DHR12R2 固定传输中点电压值 2048。当配置为双通道（DUAL 模式）时，DMA2 通道 3 向双通道寄存器 DHR12RD 循环搬运 32 位混合样本数据，单次搬运即可同步更新两路 DAC 寄存器的输出值。DMA 硬件传输配置为循环（circular）模式。

\textbf{定时触发时序：}DAC 的数模转换触发源采用 TIM6 的溢出更新事件（TRGO）。为产生频率为 $f_{sig}$ 的正弦波，DAC 触发采样率需满足 $f_s = f_{sig} \times 128$。以默认的 50 Hz 交流信号为例，DAC 的更新触发频率为 6400 Hz，定时器时钟计数值为 $N = 72{,}000{,}000 / 6400 = 11250$。设置定时器 PSC 为 0，ARR 寄存器为 11249 即可输出标准正弦信号。
...
第三层（显示缓冲帧）：参数计算完成后将工作缓冲复制到 s\_adc\_display\_frame。LCD 波形绘制、串口 STATUS 输出和 CAL ZERO 调零校准统一读取该只读缓冲区，保证数据一致性。
...
\textbf{STATUS? 查询：}主机发送 STATUS? 命令帧，设备调用函数读取全局状态数据（app\_monitor\_state\_read），调用 app\_command\_format\_status 将所有电参量格式化为一行文本，封装为 TYPE=0x81 响应帧返回。格式化使用 snprintf，各参数的小数位数格式化统一：VRMS 两位小数（0.01 V）、IRMS 和 ILK 三位小数（0.001 A）、P 一位小数（0.1 W，带符号）、S 一位小数（0.1 VA）、PF 三位小数、CAL 显示 YES/NO。

\textbf{自动上报：}开启 REPORT ON 后，主循环每秒检查一次 take\_report 信号（由 app\_capture\_task 按上报周期设置），触发时读取全局状态数据并格式化 STATUS 行，封装为 TYPE=0x82 事件帧发送。上位机侧无需逐条发送查询命令即可持续接收设备状态更新。
...
\textbf{（2）DAC 环回测试。}将 DAC 输出（PA4/PA5）通过挑线连接到 ADC 输入（PC0/PC3），发送 DAC SET MODE DUAL、DAC SET FREQ 50、DAC SET AMP 1500 等命令配置测试信号，观察 ADC 采样波形和电参量。环回测试前应确认 CAL ZERO 的执行条件。CAL ZERO 用于记录三通道 ADC 的直流零点，建议在无交流输入或输入稳定为静态中点时执行。若在带有交流信号、非整周期信号或偏置异常状态下执行，可能引入零点误差，导致后续 VRMS、IRMS、P 和 PF 计算产生偏差。
...
联机运行后，串口发送 STATUS? 获取系统状态，并与 LCD 页面显示结果对比。若两端 VRMS、IRMS、P、S、PF 和频率显示一致，说明 LCD 和串口均读取同一全局状态数据，数据链路保持一致。开启 REPORT ON 后，连续观察自动上报数据，记录是否存在丢帧、乱码、卡死或显示异常。
...
\textit{图 14\quad miniSTM32 与 miniIO 联机接线示意图}\\[0.3cm]
（接线图占位：展示 MiniSTM32 开发板 JP1 排针信号与 miniIO 副板的硬件引脚连接关系）
\vspace{3cm}}}
...
\centering
\fbox{\parbox{0.8\textwidth}{\centering\vspace{3cm}
\textit{图 15\quad 联机调试实物及接线全景照片}\\[0.3cm]
（全景实物照片占位：展示 MiniSTM32 开发板、miniIO 副板、LCD 屏、USB 串口连接及负载接线实物）
\vspace{3cm}}}
\caption{联机调试全景}
...
（2）ADC 采样数据竞争。现象：LCD 显示的电参量偶尔跳变，串口 STATUS 返回的 ADC 样点范围不一致。根因是 DMA ISR 正在写入缓冲区时主循环同时在读取。解决办法：设计了三重缓冲数据隔离机制——中断写双缓冲、主循环在极短临界区内提取就绪帧号、以及计算完毕后复制到显示缓冲帧——从而在根本上消除了数据竞争。
...
（4）CAL ZERO 执行时机不当导致电参量异常。现象：在 DAC 输出交流信号时执行 CAL ZERO 后，VRMS/IRMS/P 等参数出现明显偏差。根因是校准函数在带有交流信号的状态下记录了非静态零点，引入零点误差，导致后续 RMS 和功率计算偏差。解决办法：在报告文档与帮助信息中明确 CAL ZERO 应在无交流输入或输入稳定为静态中点时执行。

\textbf{心得体会：}

本次课程设计中最具挑战性的部分是数据竞争问题的排查——DMA 中断与主循环并发访问同一缓冲区的问题在代码审查中难以发现，需要结合现象反复推理数据时序。三重缓冲隔离机制的设计过程加深了我对嵌入式并发数据一致性的理解。

在团队协作方面，小组成员针对引脚分配、协议帧格式以及滤波算法的选择展开了充分讨论，通过将系统配置常量集中定义于 \texttt{app\_config.h}、协议帧结构规范化定义于 \texttt{app\_protocol\_defs.h}，有效保障了下位机固件与上位机分析工具之间的通信一致性。在调试及分析过程中，AI辅助工具在分析并发数据竞争根因、排查栈溢出硬故障等复杂问题上提供了有益的架构建议与分析思路，但针对具体的硬件寄存器操作、边界判定以及临界区保护，仍需人工进行细致的代码审查与测试验证。
