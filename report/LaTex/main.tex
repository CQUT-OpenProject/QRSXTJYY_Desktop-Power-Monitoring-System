...
本课程设计基于 STM32F103RCT6 微控制器，设计并实现了一套桌面用电监控系统。系统采用裸机架构，基于前后台轮询机制进行任务调度；软件设计遵循模块化原则，利用 PWM 输出产生测试频率，通过定时器输入捕获功能测量频率，使用双通道 DAC 生成测试正弦波，并通过三通道 ADC 对外部交流信号进行准同步顺序扫描采样，进而计算电参量；本地通过 TFT LCD 多级菜单进行交互，远程则基于 USART 传输二进制帧协议。

系统以 MiniSTM32 开发板为硬件核心。在信号发生与感知部分，程序通过 PA8/TIM1 输出占空比为 50\% 且频率可调的方波，并通过 PA1/TIM2 的输入捕获功能测量外部信号频率；同时，PA4/PA5 DAC 通道配合 DMA 循环搬运，输出 128 点正弦波信号。在交流信号采集方面，TIM3 定时触发三通道 ADC（PC0/PC3/PC2），以 6400 Hz 的采样率对电压、电流和漏电电流进行顺序扫描采样。三路通道共用同一触发源，通道间的转换时差极小，不影响 50 Hz 基波信号的电参量计算。DMA 将采样数据传输至工作缓冲区后，主循环调用整型算法计算电压与电流有效值、有功功率、视在功率及功率因数。通信协议采用分层设计，传输层以 A5 5A 帧头与 CRC-16/Modbus 校验保障传输可靠性，命令层则在帧内承载 ASCII 文本指令（支持 HELP、STATUS?、REPORT ON/OFF、PWM SET、DAC SET 等命令）。人机交互界面通过多页面状态机实现，包含仪表盘首页、测试菜单与四个功能子页面，且 LCD 显示与串口查询共享同一套全局状态数据，以确保两端显示数据的一致性。

系统调试表明，频率显示分辨率为 0.01 Hz，实际测量精度受定时器量化、输入频率和时钟源影响。PWM 设定范围为 1--100000 Hz，DAC 双通道频率在 1--1000 Hz、幅值参数在 0--2047 内（表示围绕 2048 中点的峰值码值偏移）均可独立调节。在联机运行测试下，ADC 采样、电参量计算、显示刷新及串口通信等模块均运行稳定，实现了课程设计的各项功能要求。
...
本系统以 STM32F103RCT6 微控制器为主控，通过协调定时器、DAC、ADC、DMA 和 USART 等片上外设，实现测试信号发生、频率测量、交流采样、电参量整型计算及串口通信等功能。

\subsectiontitle{2.1}{系统需求与功能边界}
\phantomsection\label{sec:analysis-function}

本系统旨在实时监控、分析并展示桌面用电环境中的关键电学参数，并支持信号发生、采集、处理与远程调试。在部署上，系统功能边界被划分为微控制器控制端、硬件前级信号调理端以及上位机监测端三部分。
...
\begin{itemize}
  \item \textbf{微控制器控制端（STM32 主控）：} 运行系统主循环，协调控制片上定时器、DAC 及 ADC 进行信号发生与数据采集；在本地执行定点数电参量整型算法；驱动 TFT LCD 屏幕以展示状态波形并响应多级菜单交互；同时负责接收并解析串口二进制通信帧，维护系统整体运行状态。
  \item \textbf{硬件前级信号调理端（miniIO 副板）：} 实现高压用电环境与微控制器之间的安全隔离与信号调理。利用电压与电流互感器隔离高压市电，将双极性的交流大信号等比例缩小并实施低通滤波；借助 1.65 V 参考电压源抬升信号直流偏置，将双极性交流信号转化为 0--3.3 V 的单极性安全信号以接入主控 ADC 通道；同时将交流市电整形为低压方波信号供主控测频，并提供继电器控制电路以便主控通过 GPIO 引脚安全控制强电通路。
  \item \textbf{上位机监测端（PC 测试工具）：} 实现系统的远程状态监测与参数配置。通过串口发送符合协议帧规范的控制指令，对终端状态及参数进行全量轮询，或直接监听终端主动上报的事件帧，解析数据并进行直观呈现。
\end{itemize}

\subsectiontitle{2.2}{系统性能指标}
\phantomsection\label{sec:analysis-performance}

本设计针对测量精度、实时性及人机交互流畅度要求，为各功能模块制定了如下性能指标：

\itemline{1}{信号发生指标}
\begin{itemize}
  \item \textbf{测试方波信号（PWM）：} PA8 通道输出 50\% 固定占空比方波测试信号，其频率调节范围为 1 Hz 至 100 kHz，频率设置分辨率为 1 Hz。
  \item \textbf{测试正弦波信号（DAC）：} PA4/PA5 通道独立输出单/双路正弦波信号，频率调节范围为 1 Hz 至 1000 Hz，其中幅值参数 $A=0\text{--}2047$ 表示围绕 2048 中点的峰值码值偏移量（在 $A=2047$ 时输出接近 0--3.3 V 的满幅峰峰值范围），初始相位调节范围为 0--359 度。
\end{itemize}

\itemline{2}{采样与测量指标}
\begin{itemize}
  \item \textbf{频率测量：} 测频模块支持输入信号周期捕获，其显示分辨率为 0.01 Hz，实际测量精度受定时器量化误差、输入信号频率及主控时钟源偏差的影响。针对 50 Hz 左右的电网信号，周期捕获的硬件时间误差需限制在 $\pm$1 $\mu$s 以内。
  \item \textbf{模拟通道采集（ADC）：} ADC 规则采样组采用顺序扫描模式，采集电压、电流和漏电三路调理信号，单通道采样分辨率为 12 位（对应码值 0--4095）。系统采样率设定为 6400 Hz。针对 50 Hz 基波，单周期采样样点数设计为 128 点，以满足电参量有效值（RMS）离散积分计算对奈奎斯特采样定理的要求；在实际市电频率偏移时，系统进入准同步采样状态，非整周期采样带来的计算误差通过零偏自校准和整周期近似处理进行抑制。
\end{itemize}

\itemline{3}{分析与显示指标}
\begin{itemize}
  \item \textbf{电参量测量分辨率：} 电参量的显示分辨率根据测量精度与直观呈现要求进行设计，具体为：交流有效电压 0.01 V、交流有效电流 0.001 A、有功功率 0.1 W。
...
为确保桌面用电监控系统在各种工况下安全、可靠且不间断运行，本设计在硬件、软件以及运行时序上引入了以下约束条件：

\itemline{1}{硬件与计算资源约束}
\begin{itemize}
  \item \textbf{无硬件浮点单元（No FPU）：} 主控芯片 STM32F103RCT6 搭载 Cortex-M3 内核，不具备硬件浮点运算单元。为了保证系统调度的实时性，核心算法（包括正弦波缩放与交流电参量 RMS 计算等）全部采用定点数及整型算术运算，避免使用耗时的浮点运算。
  \item \textbf{静态存储约束：} 为了保证系统长期运行的稳定性，软件设计禁用了堆空间与动态内存分配，所有缓冲区、状态结构体及通信队列均在编译期声明为静态全局变量。
\end{itemize}

\itemline{2}{信号电气安全约束}
\begin{itemize}
  \item \textbf{ADC 输入电压安全约束：} 微控制器片上 ADC 的输入电压安全范围为 0--3.3 V。由于交流市电为双极性信号，直接输入会损坏芯片。前级调理电路通过叠加 1.65 V 的直流偏置将其转换为安全的单极性信号。软件计算时以该中点电压为基准进行数字去直流（de-biasing）处理，还原出真实的交流波形。
\end{itemize}

\itemline{3}{实时性与通信时序约束}
\begin{itemize}
  \item \textbf{算法实时计算约束：} 交流电参量积分以网频周期为单位进行。为了防止数据传输阻塞与接收缓冲区溢出，计算算法必须在采样缓冲区填满后的短时间内（下一次采样触发前）完成去直流、累加平方和与开方运算。
  \item \textbf{串口通信鲁棒性约束：} 串口数据解析基于接收中断与环形缓冲区。系统引入了 300 ms 的半帧接收超时保护机制，并在检测到帧头错误或校验失败时立即重置解析状态，以防止残缺帧与信道噪声导致解析器死锁。
\end{itemize}
...
本系统围绕 STM32F103RCT6 主控芯片构建，其整体架构、硬件接口分配与软件层次设计如下。

\subsectiontitle{3.1}{系统整体设计}
\phantomsection\label{sec:design-system}

本系统以 STM32F103RCT6 微控制器为主控。系统上电后，通过引导程序和初始化函数依次配置板级外设安全电平，初始化 LCD 屏、按键输入接口与串口通信协议栈，配置测频与 PWM 定时器，并初始化 DAC、ADC 与 UI 菜单状态机，随后进入主循环。

主循环基于轮询机制按固定顺序调度各功能任务，执行流程包括：获取微秒级系统时间戳、解析串口数据并分发指令、处理测频超时与事件节流、读取 ADC 双缓冲并调用算法计算电参数、执行状态周期性上报、扫描按键并驱动菜单状态机、读取全局状态数据快照，以及刷新 LCD 显示。每一轮调度循环均以相同的时间戳 \texttt{now\_us} 为基准，以避免多模块调度过程中的时序冲突与数据源不一致。
...
开发板的 PC0--PC3 和 PC13 引脚与板载触摸屏（T\_SCK/T\_PEN/T\_MISO/T\_MOSI/T\_CS）复用。程序在初始化时向 PC13 输出高电平以禁用触摸屏片选。但受硬件上拉电阻的持续影响，系统最终弃用受干扰的 PC1 通道（即 ADC1\_IN11，原触摸屏 T\_PEN 引脚），并将电流采样改在 PC3 通道（ADC1\_IN13）进行。同理，PA2 和 PA3 引脚也输出高电平以关闭板载 SPI Flash 与 SD 卡片选，从而排除总线干扰信号。

由于开发板的 PB3 与 PB4 引脚用作 LCD 数据线，程序在启动时禁用 JTAG 功能（仅保留 SWD），以防止引脚资源冲突。
...
% 连接线(信号功能参见表\ref{tab:pin-alloc}、表\ref{tab:miniio-iface})
...
本系统软件采用分层模块化架构，自底向上划分为启动代码层、板级支持层(BSP)、硬件驱动层、核心应用逻辑层以及用户界面层。各应用模块（如 \texttt{app\_pwm}、\texttt{app\_adc}）仅通过全局状态结构体 \texttt{app\_monitor\_state\_t} 共享数据，各外设配置均通过 \texttt{app\_config.h} 的宏定义集中管理，避免了模块间的直接相互调用，保证了高内聚与低耦合。
...
app\_protocol：二进制帧传输模块，负责数据解析、发送、CRC校验及环形缓冲区管理
...
app\_dac：正弦波输出模块，配置 TIM6/TIM7 触发 DMA2 搬运，并驱动 DAC 双通道输出
...
(4)运行采样处理任务 \texttt{app\_adc\_task()}，该函数返回是否有新的 ADC 数据帧就绪。若有新帧，则将其载入主循环的工作缓冲区，调用整型算法库计算最新的电压、电流等电参数。处于 Dashboard 页面时，每 17 帧 ADC 数据触发一次屏幕刷新(约 3 Hz)，实现波形与参数的准实时显示。
...
(7)调用全局数据读取函数 \texttt{app\_monitor\_state\_read()} 获取状态数据，若置位了屏幕刷新标志 \texttt{display\_changed}，则调度屏幕显示任务 \texttt{app\_display\_task()} 刷新 LCD。LCD 刷新采用纯事件驱动机制，仅在有实际状态变化或 ADC 新帧（仪表盘页面）时触发，不使用固定周期定时刷新。
...
(4)软件资源开销与内存预算。主控芯片 STM32F103RCT6 仅有 48 KB RAM，为此软件架构全部采用静态内存分配（禁用堆空间）。具体预算估算如下：正弦波 128 点 16 位满幅查找表（256 字节，存放在 Flash 中）；两通道独立正弦波 DMA 环形缓冲区共计 512 字节（存放在 RAM 中）；ADC 规则采样通道的中断双缓冲及工作计算缓冲共计 2304 字节（3$\times$128$\times$3$\times$2 字节，存放在 RAM 中）；串口接收缓冲区及命令响应缓冲约 512 字节；以及全局状态结构体和命令响应结构体。总静态 RAM 占用约为 4 KB（约占芯片总容量的 8.3\%），保障了系统高可靠运行。此外，ADC 采样时钟分频取 PCLK2/6 = 12 MHz（满足最大 14 MHz 频率限制），以确保转换的准确性与时序稳定。

\subsectiontitle{3.4}{通信协议设计}
\phantomsection\label{sec:protocol-design}

本系统设计了一套二进制帧结构串行通信协议，用于远程监控与系统参数配置。该协议分为两层：传输层（定义帧结构与 CRC 校验）与命令层（承载具体的文本命令）。
...
DAC SET CH2 FREQ \textit{hz} & 主机$\rightarrow$设备 & 设置 DAC 通道2的输出频率(1--1000 Hz) \\
...
频率测量模块由测试频率信号发生与外部信号频率检测两部分组成。其中测试信号通过 PA8/TIM1\_CH1 输出 PWM 方波，并支持通过 LCD 菜单或串口指令调节频率。频率测量则通过 PA1/TIM2\_CH2 输入捕获相邻上升沿的时间差来计算频率。两部分均通过统一的二进制帧协议与上位机进行通信。

\subsectiontitle{4.1}{测试频率生成}
\phantomsection\label{sec:frequency-generate}

TIM1 通道 1 (PA8) 输出的 PWM 测试信号配置为 PWM1 模式、向上计数、50\% 固定占空比。调节频率时，关键在于根据目标频率计算定时器的预分频器 PSC 和自动重装载值 ARR。
...
程序配置 TIM2 的计数时钟为 1 MHz（系统时钟 72 MHz 进行 72 分频），并将 ARR 寄存器设为 0xFFFF，使其以自由递增模式运行，溢出后自动清零。通道 2 的输入捕获采用上升沿触发方式，数字滤波系数设为 0x03（即输入信号连续 4 个采样时钟周期保持一致时才确认边沿，以滤除杂波干扰）。

\textbf{测量与时间戳扩展原理：}当输入引脚检测到信号上升沿时，触发捕获中断并读取 16 位捕获寄存器 CCR2 的值。为了防止 16 位定时器溢出限制测频范围，程序设计了时间戳扩展函数 \texttt{make\_capture\_time()}，将 16 位捕获值拼接为 32 位微秒级时间戳：低 16 位取自 CCR2 寄存器，高 16 位由累加溢出计数器 \texttt{s\_overflows} 产生。在读取瞬间，若定时器溢出中断标志（Update 标志）已置位且当前捕获值较小（小于 32768），说明捕获事件发生在计数器溢出之后但溢出中断服务尚未执行，此时软件对高 16 位时间戳自动补偿加 1，以修正计数偏差。

相邻两次捕获得到的 32 位时间戳之差即为所测信号的物理周期 $T$（单位 $\mu$s），对应的输入频率为：

$$f \times 100 = 100{,}000{,}000 / T$$

计算出的频率值以 $0.01$ Hz 为显示分辨率存入整型变量，其物理测量精度受 1 $\mu$s 定时器时基量化误差的制约。

\textbf{超时与上报节流管理：}主循环在 \texttt{app\_capture\_task()} 中检测自上一次捕获以来流逝的时间，以防输入信号丢失后仍显示过期频率。若超过 2 秒未检测到有效捕获边沿，系统会将测量频率归零并清除就绪标志。同时，为了避免频率频繁波动导致串口拥堵，上报机制实现了周期性（每秒一次）与触发式（频率变化时立即上报，并限制最小间隔为 200 ms）相结合的节流控制。
...
\begin{tikzpicture}[node distance=6mm and 14mm, >=Latex]
  ...
  \node[blk, below=of c5] (c6) {以 Hz$\times$100 存储\\(0.01\,Hz 显示分辨率)};
  \draw[fl] (c1)--(c2);
  \draw[fl] (c2)--(d);
...
交流采样模块利用定时器触发 ADC 进行模数转换，并通过 DMA 将采样数据传送至内存。同时，系统具备双通道 DAC 正弦波发生能力，可用作环回测试信号源以验证采样与计算链路。

\subsectiontitle{5.1}{测试电压电流生成与实际信号来源}
\phantomsection\label{sec:adc-generate}

本设计提供了双重交流信号源，以适应调试与实际应用的需求。第一种是利用内置 DAC 产生的正弦波测试信号，用于在不连接 miniIO 副板和高压负载时，闭环验证单片机自身的采样、DMA 传输以及电参量计算算法。第二种是 miniIO 副板的实际采样输出，包含调理后的电压信号 VL、电流信号 iL 和漏电流信号 iLK 以及整形频率信号 fr。在系统实际接入桌面负载时，系统需要连接至 miniIO 输入源。

测试正弦波信号由 PA4 与 PA5 双通道 DAC 硬件输出。正弦波发生方案采用闭环设计：在内存中建立 128 点查找表，由定时器硬件触发，并通过 DMA 循环传输至 DAC 寄存器。
...
其中幅值参数 $A_1$、$A_2$ （范围 0--2047）代表围绕 2048 中点的峰值码值偏移。当参数设定为 2047 时，输出的正弦信号能够达到 0--3.3 V 的最大满幅峰峰值。运算结果在输出前进行限幅，以防数据溢出。
...
\textbf{DMA 硬件传输：}系统始终采用双 DMA 通道传输，以实现两通道独立频率输出。DMA2 通道 3 负责将通道1正弦波样本（\texttt{s\_dac\_ch1}）搬运至 DAC 通道1右对齐寄存器 \texttt{DHR12R1}，DMA2 通道 4 负责将通道2样本（\texttt{s\_dac\_ch2}）搬运至 DAC 通道2右对齐寄存器 \texttt{DHR12R2}。当配置为单通道正弦（SINGLE 模式）时，通道2的样本缓冲区 \texttt{s\_dac\_ch2} 固定填充为中点电压值 2048（\texttt{APP\_DAC\_MID\_CODE}）；当配置为双通道正弦（DUAL 模式）时，\texttt{s\_dac\_ch2} 填充为计算好的通道2正弦波数据。DMA 硬件传输配置为循环（circular）模式。
...
ADC 采样链路基于硬件协同实现：由 TIM3 定时触发（6400 Hz TRGO）ADC1 顺序扫描三通道，采样结果由 DMA1 通道 1 循环传输至内存。
...
\textbf{三重缓冲隔离机制：}为了解决 DMA 中断、主循环处理以及 LCD/串口输出之间的数据竞争问题，系统设计了三重缓冲隔离机制：

第一层（ISR 写缓冲）：在 DMA HT/TC 中断中，\texttt{deinterleave\_frame()} 从 DMA 缓冲区将交错数据分离到双缓冲 \texttt{s\_adc\_frame[2]} 的写侧，并在分离完成后切换读写索引。

第二层（主循环工作帧）：\texttt{app\_adc\_task()} 在极短临界区（仅关闭中断读取就绪帧编号并清除标志）内快速执行，随后在临界区外（不阻塞中断）将整帧数据从 ISR 缓冲区复制到工作帧 \texttt{s\_adc\_work\_frame} 中，最后调用算法库计算电参量。

第三层（显示缓冲帧）：参数计算完成后，程序将工作缓冲区复制到 \texttt{s\_adc\_display\_frame} 中。LCD 波形绘制和串口 STATUS 输出统一读取此只读缓冲区，以确保两端数据显示一致。
...
\textbf{刷新策略：}LCD 刷新采用纯事件驱动机制，仅在置位 display\_changed 标志时重绘当前页面，不再使用固定周期定时刷新。在 Dashboard 仪表盘页面，每 17 帧 ADC 新数据触发一次刷新（约 3 Hz），实现波形准实时更新；其余页面仅在串口命令、按键操作等状态变化时触发刷新。换页时先清除内容区域避免旧页面残留。
...
电参量计算是系统的核心功能之一，主要负责将 ADC 采集的原始采样点换算为实际的电气物理量。
...
\textbf{(1)直接计算方法。}对采样得到的离散时域信号直接进行数学运算。电压有效值(RMS)定义为 $V_{RMS} = \sqrt{\frac{1}{N}\sum_{i=0}^{N-1}(V_i - V_{dc})^2}$，有功功率定义为 $P = \frac{1}{N}\sum_{i=0}^{N-1} (V_i - V_{dc}) \times (I_i - I_{dc})$（其中 $V_{dc}$ 和 $I_{dc}$ 为各自通道的直流分量，即计算时已扣除直流偏置）。该方法计算简单、实时性好，对采样率要求不高，适合嵌入式系统。
...
\textbf{(1)DC 偏移确定。}系统实现了零偏自校准算法，以扣除输入信号叠加的 1.65V 直流偏置。在上电启动时，程序采集第一帧（128点，包含完整工频周期）数据并计算其算术平均值作为各通道的直流偏置零点，并在随后的运行中动态扣除，从而有效消除了电阻分压和运放器件失调带来的硬件容差，实现无负载时电压、电流和功率读数的精准归零。
...
其中 $G_v$、$G_i$ 为将物理换算系数放大 $10^3$ 倍后的整型增益。电压通道增益 $G_v = 17600$ 对应实际电压互感器阻抗分压比与 ADC 采样的换算比例。校准过程如下：在系统接通市电后，使用数字万用表测得交流有效电压的实际测量值 $V_{meter}$（例如 220.0 V），同时读取系统未校准时的去直流 RMS 码值 $\text{RMS}_{code}$（例如 1250），根据换算关系 $V_{meter} = \text{RMS}_{code} \times \frac{G_v}{1000 \times 100}$，反推出校准增益 $G_v = 100000 \times \frac{V_{meter}}{\text{RMS}_{code}} \approx 17600$。电流通道增益 $G_i$ 亦可通过接通标准纯阻性负载，由类似方法通过已知电流或负载阻值反推标定（本系统默认设为 1000，即 1:1 分流比关系）。电压增益按实测比值整定，可通过修改 \texttt{APP\_ADC\_CALIB\_V\_GAIN\_X1000} 宏调整，无需重新编写算法。
...
$$P \times 10 = \frac{P_{code} \times G_v \times G_i}{10^{10}}$$

在定点数计算中，电压校准增益 $G_v$ 与电流校准增益 $G_i$ 均在程序中放大了 $10^3$ 倍（即以毫倍数存储）。计算有功功率时，交流信号采样值乘积 $v_{ac} \times i_{ac}$ 对应引入了 $G_v \times G_i$ 的 $10^6$ 倍放大。同时，由于电压有效值单位为 0.01 V（放大 100 倍），电流有效值单位为 0.001 A（放大 1000 倍），若要将计算得到的有功功率转换为 0.1 W 为单位的物理量（放大 10 倍），其合并物理换算系数为 $10^2 \times 10^3 / 10 = 10^4$。因此，为抵消增益放大并完成单位换算，公式中除以的总因子为 $10^6 \times 10^4 = 10^{10}$。结果保留符号，正值表示消耗功率，负值表示回馈功率。
...
系统通过串口协议的 \texttt{STATUS?} 命令和定时自动上报功能，将电参量数据传输至上位机。通信协议的具体设计详见第~\ref{sec:protocol-design}~节。

\textbf{自动上报逻辑：}当开启自动上报（通过指令 \texttt{REPORT ON}，见表~\ref{tab:system-commands}）且触发标志 \texttt{take\_report} 置位时，主循环读取最新的全局电参量计算快照，将其格式化为 STATUS 数据行，并封装为 \texttt{TYPE=0x82} 的事件帧主动推送至上位机。该机制显著降低了上位机的轮询开销，提高了数据传输效率。
...
为了验证各模块功能与系统的协调性，本设计对通信协议、测频、交流采样及电参量计算算法进行了联调测试。

\subsectiontitle{7.1}{通信协议测试}
\phantomsection\label{sec:debug-protocol}

通信协议测试针对二进制帧的帧头识别、长度解析、命令处理、CRC 校验及错误帧拦截等功能进行正确性与可靠性验证。
...
测试结果表明，协议解析器可准确识别合法帧并过滤异常帧，满足系统参数配置与状态上报的要求。
...
\textbf{(1)零信号基线测试。}不接入外部信号，发送 STATUS? 查看各通道 ADC 原始码值范围。在无信号输入时，PC0 和 PC3 的原始采样码值应稳定在中点附近（约 2048），具体测试数据如表~\ref{tab:adc-raw} 所示。若某通道码值明显偏大或偏小，说明外部电路拉偏了引脚。

\textbf{(2)DAC 环回测试。}将 DAC 输出(PA4/PA5)通过跳线连接到 ADC 输入(PC0/PC3)，发送 DAC SET MODE DUAL、DAC SET CH1 FREQ 50、DAC SET CH1 AMP 1500 等命令配置测试信号，观察 ADC 采样波形和电参量。理论中点为 2048，实际运行时以上电首帧均值锁定各通道零偏，因此各通道即使静态偏置偏离 2048，也可通过软件去直流后参与 RMS 计算。
...
电参量调试主要通过 DAC 环回输出已知正弦信号，验证电压有效值（$V_{RMS}$）、电流有效值（$I_{RMS}$）、有功功率（$P$）、视在功率（$S$）和功率因数（$PF$）的计算趋势。

\textbf{(1)幅值线性验证。}我们将 DAC 两通道的输出频率固定为 50 Hz，初始相位均设为 0°，通过 \texttt{DAC SET CH1 AMP} 和 \texttt{DAC SET CH2 AMP} 逐步改变 DAC 输出幅值（500、1000、1500、2000），并记录各项电参量（见表~\ref{tab:amp-linear}），以验证电压与电流有效值是否随幅值呈线性增长。注：在此次 DAC 环回测试中，算法采用单位增益以验证趋势，计算得到的 VRMS 和 IRMS 为归一化的 “码值/100”，而非乘上市电校准增益 $G_v = 17600$ 后的真实高压数值。
...
\textbf{(4)零信号电平验证。}无信号输入时，由于系统已固定扣除直流偏置，各通道的 VRMS 和 IRMS 值应接近 0。
...
(1)上电后串口收到事件帧 ``OK DEVICE READY''，确认固件启动正常。
...
(3)继电器控制异常。现象：联机后继电器无法正确吸合，导致无法测得实际市电参数。排查发现初始化函数 \texttt{app\_init()} 中遗漏了对 \texttt{app\_relay\_init()} 的调用，导致未配置 PC12 引脚。解决办法：在 \texttt{app\_init()} 中补全继电器初始化调用，确保上电默认安全断开，并在切入 Dashboard 仪表盘页时正确吸合。
