...
测试用正弦波信号由 PA4/PA5 通道的双路 DAC 产生。系统内部预先定义了 128 点全幅正弦查找表。在运行期间，程序根据通道1和通道2分别设定的幅度、相位参数对查找表数据进行缩放与移位，并写入各自的 DMA 缓冲区。由 TIM6 和 TIM7 分别作为触发源，驱动两个通道 of DMA 循环将数据搬运至各自的 DAC 寄存器输出。该模块支持通道1与通道2各自独立的频率、幅值和相位调节，同时支持单通道正弦（另一通道输出 1.65V 中点电压）及双通道正弦两种模式。两通道的输出频率在 1--1000 Hz 范围内可调，幅度码值在 0--2047 范围内可调。
...
系统采用两层协议设计。传输层定义二进制帧结构：A5 5A 帧头 + 版本号 + 帧类型 + 序号 + 长度 + Payload + CRC-16/Modbus 校验，最大 Payload 512 字节（支持大段状态数据）。帧类型包括 0x01(命令)、0x81(响应)、0x82(事件/自动上报)、0x83(协议错误)。命令层在帧 Payload 中承载 ASCII 文本指令，支持 HELP、STATUS?、REPORT?、REPORT ON/OFF、PWM SET、DAC SET MODE、DAC SET CH1/CH2 等命令，不区分大小写。
...
STM32F103RCT6 提供 256 KB Flash 和 48 KB RAM，系统时钟 72 MHz。系统不使用堆分配，所有缓冲区均为静态数组。DAC 满幅查找表（256 字节，存放在 Flash）与两路独立 DMA 输出缓冲（512 字节，存放在 RAM）共约 768 字节，ADC DMA 缓冲区 768 半字(1536 字节)，协议环形缓冲 256 字节，加上命令响应结构体(4$\times$256 = 1024 字节)，总静态内存占用约 4 KB，远低于 48 KB 上限。ADC 时钟需 $\leq$ 14 MHz，实际取 PCLK2/6 = 12 MHz。
...
(3)DAC 正弦波生成算法。预计算 128 点满幅正弦查找表 $S[n] = 2048 + 2047 \cdot \sin(2\pi n / 128)$，运行时按两通道各自的幅度参数 $A_1, A_2$ 缩放：$S'[n] = 2048 + (S[n] - 2048) \times A / 2047$。通道1与通道2均可独立引入相位偏移索引 $\Delta_1 = \phi_1 \times 128 / 360$ 和 $\Delta_2 = \phi_2 \times 128 / 360$。通道1由 TIM6 触发，触发采样率为 $f_{s1} = f_{sig1} \times 128$；通道2由 TIM7 触发，触发采样率为 $f_{s2} = f_{sig2} \times 128$，通过调整各自定时器的 PSC 和 ARR 实现。

(4)ADC 采样与数据拆分。ADC 采用同一触发源下的三通道准同步顺序扫描、DMA 循环搬运与中断解交错的数据采集方案，详见第~\ref{sec:adc}~章。
...
DAC SET CH1 AMP \textit{code} & 主机$\rightarrow$设备 & 设置 DAC 通道1幅度码值(0--2047) \\
DAC SET CH1 FREQ \textit{hz} & 主机$\rightarrow$设备 & 设置 DAC 通道1输出频率(1--1000 Hz) \\
DAC SET CH1 PHASE \textit{deg} & 主机$\rightarrow$设备 & 设置 DAC 通道1相位偏移(0--359 度) \\
DAC SET CH2 AMP \textit{code} & 主机$\rightarrow$设备 & 设置 DAC 通道2幅度码值(0--2047) \\
DAC SET CH2 FREQ \textit{hz} & 主$\rightarrow$设备 & 设置 DAC 通道2输出频率(1--1000 Hz) \\
DAC SET CH2 PHASE \textit{deg} & 主机$\rightarrow$设备 & 设置 DAC 通道2相位偏移(0--359 度) \\
OK DAC MODE=\ldots & 设备$\rightarrow$主机 & DAC 模式设置响应 \\
OK DAC CH1 AMP=\ldots & 设备$\rightarrow$主机 & 通道1幅度设置响应 \\
OK DAC CH2 AMP=\ldots & 设备$\rightarrow$主机 & 通道2幅度设置响应 \\
OK DAC CH1 FREQ=\ldots Hz & 设备$\rightarrow$主机 & 通道1频率设置响应 \\
OK DAC CH2 FREQ=\ldots Hz & 设备$\rightarrow$主机 & 通道2频率设置响应 \\
OK DAC CH1 PHASE=\ldots & 设备$\rightarrow$主机 & 通道1相位设置响应 \\
OK DAC CH2 PHASE=\ldots & 设备$\rightarrow$主机 & 通道2相位设置响应 \\
\bottomrule
\end{tabular}
\end{table}
...
<< RSP OK REPORT=ON
--- 继续监听 5 秒自动上报 ---
<< EVT OK STATUS MEAS=50.00Hz PWM=50Hz REPORT=ON DAC_MODE=DUAL DAC_FREQ1=50Hz DAC_FREQ2=50Hz DAC_AMP1=1500 DAC_AMP2=1500 DAC_PHASE1=0 DAC_PHASE2=0 VRMS=10.60 IRMS=1.060 ILK=0.394 P=11.2 S=11.2 PF=1.000
<< EVT OK STATUS MEAS=50.00Hz PWM=50Hz REPORT=ON DAC_MODE=DUAL DAC_FREQ1=50Hz DAC_FREQ2=50Hz DAC_AMP1=1500 DAC_AMP2=1500 DAC_PHASE1=0 DAC_PHASE2=0 VRMS=10.61 IRMS=1.061 ILK=0.394 P=11.2 S=11.2 PF=1.000
\end{lstlisting}
